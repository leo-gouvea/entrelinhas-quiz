...
const sons = {
    swipe: new Audio('sons/cardSwipe.wav'),       // passar o mouse em cima das opções
    chegando: new Audio('sons/cardArriving.wav'), // cartões/telas chegando
    correto: new Audio('sons/cardSelect.wav'),    // resposta certa
    incorreto: new Audio('sons/error.wav')        // resposta errada
};
...
function tocarSom(audioBase, volume = 0.5) {
    // Clona o elemento de áudio para permitir sons sobrepostos (ex: hover rápido em vários cartões)
    const instancia = audioBase.cloneNode();
    instancia.volume = volume;
    instancia.play().catch(() => {
        // Navegadores bloqueiam autoplay antes da primeira interação do usuário; ignorar silenciosamente
    });
}

function playHoverSound() {
    tocarSom(sons.swipe, 0.22);
}

function playSwipeSound() {
    // Usado tanto na troca de telas quanto na chegada de cada cartão da pergunta
    tocarSom(sons.chegando, 0.3);
}

function playCorrectSound() {
    tocarSom(sons.correto, 0.4);
}

function playIncorrectSound() {
    tocarSom(sons.incorreto, 0.35);
}

function playResultSound(scorePercentual) {
    // Som neutro de "chegada" para o resultado final — não implica certo/errado,
    // já que o resultado pode ser bom ou ruim independente do som usado.
    tocarSom(sons.chegando, 0.35);
}
...
// Em dispositivos touch, mantém o cursor nativo (sem cursor customizado)
...
// CORREÇÃO: Usar delegação de eventos para evitar acúmulo de listeners
...
function mostrarTela(idTela, direcao = 'direita') {
    ...
    // Se já estamos nessa tela, não faz nada
    ...
    // Limpa possíveis estados anteriores
    novaTela.classList.remove(
        'tela-ativa',
        'entrando-direita',
        'entrando-esquerda',
        'tela-saindo-esquerda',
        'tela-saindo-direita'
    );
    ...
    /*
       ...
       Isso faz o navegador reconhecer:
       "a nova tela está fora da tela"

       antes de começar a animação.
    */
    void novaTela.offsetWidth;
    ...
    setTimeout(() => {
        ...
        // Remove completamente a tela antiga
        telaAtual.classList.remove(
            'tela-ativa',
            'tela-saindo-esquerda',
            'tela-saindo-direita'
        );
...
let podeTocarSomCard = true; // Controla se os sons dos cards podem tocar
...
async function gerarImagemResultado() {
    ...
    // Garantir que o elemento está visível e renderizado
    const modal = document.getElementById('modal-compartilhar');
    const estaVisivel = !modal.classList.contains('escondido');
    ...
    try {
        const canvas = await html2canvas(elementoResultado, {
            ...
            useCORS: true,
            allowTaint: false,
            width: elementoResultado.offsetWidth,
            height: elementoResultado.offsetHeight,
            windowWidth: elementoResultado.scrollWidth,
            windowHeight: elementoResultado.scrollHeight
        });
